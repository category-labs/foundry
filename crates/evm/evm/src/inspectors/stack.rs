...
impl InspectorStackRefMut<'_> {
    ...
    fn adjust_evm_data_for_inner_context(&mut self, ecx: &mut MonadContext<&mut dyn DatabaseExt>) {
    ...
    fn do_call_end(
        &mut self,
        ecx: &mut MonadContext<&mut dyn DatabaseExt>,
        inputs: &CallInputs,
    ...
    fn transact_inner(
        ...
        // Convert MonadCfgEnv to CfgEnv<MonadSpecId> for Env::from
        let cached_env = Env::from(ecx.cfg.clone().into(), ecx.block.clone(), ecx.tx.clone());
        ...
        let res = self.with_inspector(|inspector| {
            ...
            // Convert MonadCfgEnv to CfgEnv<MonadSpecId>
            *env.cfg = evm.cfg.clone().into();
    ...
    fn top_level_frame_start(&mut self, ecx: &mut MonadContext<&mut dyn DatabaseExt>) {
        if self.enable_isolation {
...
impl Inspector<MonadContext<&mut dyn DatabaseExt>> for InspectorStackRefMut<'_> {
    ...
    fn log(&mut self, ecx: &mut MonadContext<&mut dyn DatabaseExt>, log: Log) {
        call_inspectors!(
    ...
    fn selfdestruct(&mut self, contract: Address, target: Address, value: U256) {
        call_inspectors!([&mut self.printer], |inspector| Inspector::<
            MonadContext<&mut dyn DatabaseExt>,
        >::selfdestruct(
...
impl Inspector<MonadContext<&mut dyn DatabaseExt>> for InspectorStack {
    fn step(
        &mut self,
        interpreter: &mut Interpreter,
        ecx: &mut MonadContext<&mut dyn DatabaseExt>,
    ...
    fn call(
        &mut self,
        context: &mut MonadContext<&mut dyn DatabaseExt>,
